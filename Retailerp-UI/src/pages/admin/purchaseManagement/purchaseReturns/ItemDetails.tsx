...
const ItemDetails = ({ edit }: Props) => {
  ...
  const handleAddRow = () => {
    ...
    // If no rows exist, add the first row without validation
    ...
    // Validate last row only if rows exist
    if (
    ...
    <Grid width={'100%'} pb={2}>
      ...
      {(rows.length > 0 ? rows : defaultRow).map((row: any, index: number) => (
        <Grid container sx={tableRowStyle} key={row.id}>
          ...
          >
            <TextInput
              value={row.ordered_weight}
              onChange={(e: any) => {
                const value = e.target.value;
                if (value === '' || /^\d*\.?\d*$/.test(value)) {
                  handleInputChange(row.id, 'ordered_weight', value);
                }
              }}
              {...tableTextInputProps}
            />
          ...
          >
            <TextInput
              ...
              onChange={(e: any) => {
                const value = e.target.value;
                if (value === '' || /^\d*\.?\d*$/.test(value)) {
                  handleInputChange(row.id, 'received_weight', value);
                }
              }}
              {...tableTextInputProps}
            />
