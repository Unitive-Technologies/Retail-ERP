...
export const ProductService = {
    ...
    UpdateProduct: async ({ id, data, successMessage, failureMessage }: UpdateProductProps) => {
        const options = await apiOptions({
    ...
    delete: async (id: number | string, messages?: { successMessage?: string; failureMessage?: string }) => {
        const options = await apiOptions({
            url: `${Config.BASE_URL}${SERVICE_URL.PRODUCT}/${id}`,
            method: 'delete',
        });
    ...
    createAddons: async (data: { product_id: number; addon_product_ids: number[] }) => {
        const options = await apiOptions({
